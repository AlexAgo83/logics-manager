...
- Restored the stacked board/details splitter below `900px`, widened its drag handle, and fixed list-group collapse synchronization.
...
- Replaced the floating filter/search controls cluster with a toggleable second toolbar row for search, grouping, sorting, filter toggles, and reset, with persisted disclosure state and hidden active-control hints.
