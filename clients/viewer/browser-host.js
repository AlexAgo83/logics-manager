...
${baseEntry.stack.split("\n", 1)[0] || ""}`;
  ...
  function renderHealthSummary(lintData, auditData) {
    ...
    const cards = [
      ...
      `).join("");
    const list = findings.length ? findings.slice(0, 50).map((finding) => {
      ...
      <details class="viewer-release__gate">
        <summary>
          <span>
            ...
            <em>${escapeHtml(gate?.state || "")}${gate?.required === false ? " \xB7 optional" : ""}</em>
          </span>
  ...
  (() => {
    ...
    function recordGitActivity(action, meta2 = "") {
      ...
      history.unshift({
        ...
        label: safeAction,
        meta: meta2 || `Git ${safeAction.toLowerCase()} started`,
        at: now,
        updatedAt: now
      });
    ...
    function updateMainCdxBadge(payload) {
      ...
      if (activeSessions <= 0) {
        button.title = isCapabilityAvailable("cdx") ? "Show CDX status" : capabilityMessage("cdx", "CDX is not available for this project.");
        updateCdxUnreadBadges();
        return;
      }
    ...
    function renderUpdateNotice(updateInfo, cdxUpdateInfo) {
      ...
      if (copy) {
        copy.textContent = notices.map(({ name, info }) => `${name} ${info.latestVersion} is available. Current version: ${info.currentVersion || "unknown"}.`).join(" ");
      }
      if (command) {
...
${line}` : line;
    ...
    function persistWorkshopTerminalOrder() {
      const rootKey = workshopTerminalOrderRootKey();
      const byRoot = viewerPreferences.workshopTerminalOrderByRoot && typeof viewerPreferences.workshopTerminalOrderByRoot === "object" ? viewerPreferences.workshopTerminalOrderByRoot : {};
      updateViewerPreferences({
    ...
    async function showCdxStatus(options = {}) {
      ...
      if (options.skipUnchanged && !options.force && latestCdxStatusSignature && nextCdxSignature === latestCdxStatusSignature) {
        ...
        if (!options.silent) {
          setMeta(`Checked CDX status just now \xB7 no changes (${(/* @__PURE__ */ new Date()).toLocaleTimeString()})`);
        }
        return;
      }
    ...
    function renderCdxMemory(payload, scope = "current", viewMode = "cleaned") {
      ...
      const cards = [["Memory", cdxLabel(scope)], ["Status", cdxLabel(state)], ["Cleaned", sizeLabel], ["Noise", payload?.noise_ratio !== void 0 ? `${Math.round(Number(payload.noise_ratio || 0) * 100)}%` : "-"]].map(([label, value]) => `
      <div class="viewer-cdx__card">
        <div class="viewer-cdx__label">${escapeHtml(label)}</div>
    ...
    async function openCdxArtifact(path) {
      ...
      setDocument(data.payload?.name ? `CDX log \xB7 ${data.payload.name}` : "CDX log", renderCdxLogPreview(data.payload));
    ...
    async function resetReleaseState() {
      ...
      setMeta(cleared > 0 ? `Release evidence reset \u2014 cleared ${cleared} entr${cleared === 1 ? "y" : "ies"}; gates are pending.` : "Release evidence already empty; gates are pending.");
    }
    ...
    async function showGitStatus(options = {}) {
      ...
      if (options.skipUnchanged && !options.force && latestGitStatusSignature && nextGitSignature === latestGitStatusSignature) {
        ...
        updateMainGitBadges();
        if (!options.silent) {
          setMeta(`Checked Git status just now \xB7 no changes (${(/* @__PURE__ */ new Date()).toLocaleTimeString()})`);
        }
        return;
      }
