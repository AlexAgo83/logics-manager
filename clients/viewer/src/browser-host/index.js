...
(() => {
  ...
  //: item_810: nothing is shown below this. req_364's cache made most loads answer in
  //: about 12ms, so every affordance here was being shown and hidden inside a single
  //: frame -- an appearance the eye catches only as a glitch. One value, read by the ring,
  //: the status spinner and the header sheen alike, because three thresholds are three
   ...
   *
   * The buttons are the same elements the desktop header shows -- only their presentation
   * changes -- so choosing one runs the handler it already had. Closing on that click is
   * what makes it a menu rather than a panel that has to be dismissed twice.
   */
  function installTopbarMenu() {
    ...
    // A screen chosen from the sheet closes it; the nav menus it opens are their own.
  ...
  // item_712/item_714. Each project used to be a card with three stacked metric tiles --
  // about 360px to show three digits, so two projects filled a 1440x900 screen and one
  // filled a phone. A row carries the same facts in about 40px.
  //
  // State is carried by the left accent, and by its *shape* as well as its colour, per
  // the decision recorded in item_767: greyscale the screen and the ordering survives.
  // Zeros recede rather than being rendered at the weight of findings.
  ...
  function renderFleetRow({ project, state, favorite }) {
    ...
    const facts = counted
      ? `<span class="viewer-fleet__metrics">${renderFleetMetric("open", state.openCount ?? 0, "")}${renderFleetMetric("issues", state.issueCount ?? 0, "bad")}${renderFleetMetric("stale", state.staleCount ?? 0, "warn")}</span>`
      : `<span class="viewer-fleet__note">${escapeHtml(projectState.label)}</span>`;
    return `
  ...
  function renderSettingsIdentity(info, mcpState) {
    ...
    // item_886/adr_033: adoption is offered, never performed on its own. It adds the other
    // record's favourites and Fleet roots to this one and leaves the source file alone.
    const forked = others.length
      ? `<p class="viewer-settings-screen__hint">Another operator preferences file exists for this account: ${others.map((path) => escapeHtml(String(path))).join(", ")}. This viewer reads and writes the one above; favourites and Fleet roots kept in the other are not lost, only elsewhere.</p>
  ...
  async function showSettings(options = {}) {
    ...
    // item_849: the payload reports `running` (a boolean), never a `state` string --
    // reading `state` left the toggle permanently "unknown" whatever the connector did.
  ...
  async function showChatgptMcp(options = {}) {
    ...
    // pending view, including the one that just rendered, so each tick takes a fresh
    // silent token that yields to any newer render. What is still on screen is what
    // decides whether to keep polling.
    // ponytail: setTimeout re-entry rather than a tracked interval -- nothing to clear.
  ...
  function buildCorpusInsights(lintData = null, auditData = null) {
    ...
    const concentratedIssues = Object.entries(countBy(qualityFindings, (finding) => finding.path || "repository"))
      ...
      .slice(0, 8);
    // item_746/item_747: a signal is a defect when it describes something that cannot resolve
    // itself, and work in flight when time alone resolves it -- until it has had that time.
    // An incomplete chain is what a freshly scaffolded chain *is*; at review time every
    // document under Flow health was a chain written within the hour, so the headline was
    // counting the normal state of new work. The threshold is 14 days and item_746 records
    // it as a guess with nothing measured behind it, because no per-beat dates exist.
  ...
  async function editDocument(item) {
    ...
    // item_845: VS Code already has its own editor one message away; the standalone
    // browser has none, so only it gets the in-viewer screen. Same signal the panel
    // already uses to know it is embedded.
  ...
  //: item_845: the one active edit, if any -- so Cancel and Save know which document and
  //: which content they are working against without re-deriving it from the DOM.
  //: ponytail: clicking Edit again while this screen is open replaces it with no warning,
  //: same as opening a different document would; add a dirty-check confirm if that
  //: proves to be a real trap in practice.
  ...
  async function saveDocEditorScreen() {
    ...
    if (data.payload?.changed === false) {
      // item_846 AC2: nothing changed, so there is nothing to commit.
  ...
  //: item_844/item_846: the one place either flow commits -- a proposed commit, wired to
  //: the existing git-commit route. Throws with the server's own reason on failure.
  ...
  window.addEventListener("load", () => {
    ...
    // Pull and Push move the repository against a remote from a single click in a menu, with
    // no undo on the far side of a push. Commit already asks (openGitCommitModal); Fetch only
    // updates remote-tracking refs and stays a plain click.
