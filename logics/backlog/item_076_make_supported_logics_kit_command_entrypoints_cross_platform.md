...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|make-supported-logics-kit-command-entryp|req-025-harden-logics-kit-workflow-gener|make-the-vs-code-extension-genuinely|ac1-the-request-explicitly-covers-both
flowchart LR
    Request[req_025_harden_logics_kit_workflow_generat] --> Problem[Make the VS Code extension genuinely]
    Problem --> Scope[Make supported Logics kit command entrypoi]
    Scope --> Acceptance[AC1: The request explicitly covers both]
    Acceptance --> Tasks[task_080_make_supported_logics_kit_command]
```
...
- Primary task(s): `task_080_make_supported_logics_kit_command_entrypoints_cross_platform`
...
# Priority
- Impact: High. The shared kit command surface defines whether Windows support is real or just extension-local.
- Urgency: Medium. It should follow quickly after the extension and repo command surface are stabilized.
...
- Completed on 2026-03-19 via `task_080_make_supported_logics_kit_command_entrypoints_cross_platform`.
