...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|clean-contributor-and-release-guidance-f|req-062-harden-windows-compatibility-acr|clarify-the-documented-windows-operator-|ac1-the-request-explicitly-covers-docume
flowchart LR
    Request[req_062_harden_windows_compatibility_acros] --> Problem[Clarify the documented Windows operator pa]
    Problem --> Scope[Clean contributor and release guidance for]
    Scope --> Acceptance[AC1: The request explicitly covers documen]
    Acceptance --> Tasks[task_085_clean_contributor_and_release_gui]
```
...
- Primary task(s): `task_085_clean_contributor_and_release_guidance_for_windows_friendly_shell_and_temp_path_usage`
...
# Priority
- Impact: Medium. This reduces maintainer friction and avoids Unix-only habits leaking into repeatable release steps.
- Urgency: Low. It matters, but it can follow the runtime and CI hardening work.
