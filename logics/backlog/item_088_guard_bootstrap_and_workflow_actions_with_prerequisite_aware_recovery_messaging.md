## item_088_guard_bootstrap_and_workflow_actions_with_prerequisite_aware_recovery_messaging - Guard bootstrap and workflow actions with prerequisite-aware recovery messaging
> From version: 1.10.8
> Status: Done
> Understanding: 97%
> Confidence: 95%
...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|guard-bootstrap-and-workflow-actions-wit|req-062-harden-windows-compatibility-acr|add-an-explicit-environment-preflight-so|ac1-the-request-defines-an-explicit
flowchart LR
    Request[req_062_harden_windows_compatibility_acros] --> Problem[Add an explicit environment preflight so]
    Problem --> Scope[Guard bootstrap and workflow actions with]
    Scope --> Acceptance[AC1: The request defines an explicit]
    Acceptance --> Tasks[task_082_guard_bootstrap_and_workflow_acti]
```
...
- Primary task(s): `task_082_guard_bootstrap_and_workflow_actions_with_prerequisite_aware_recovery_messaging`
...
# Priority
- Impact: High. This is the main UX layer that turns environment detection into actionable and less surprising behavior.
- Urgency: Medium. It should follow capability detection closely, but after the prerequisite model itself is defined.
...
- Completed on 2026-03-19 via `task_082_guard_bootstrap_and_workflow_actions_with_prerequisite_aware_recovery_messaging`.
