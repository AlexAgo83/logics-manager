## item_170_add_plugin_in_progress_feedback_for_ollama_backed_hybrid_assist_actions - Add plugin in-progress feedback for Ollama-backed hybrid assist actions
> From version: 1.14.0
> Schema version: 1.0
> Status: Ready
> Understanding: 96%
...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|add-plugin-in-progress-feedback-for-olla|req-100-add-user-feedback-and-vs-code-no|give-the-operator-immediate-visible-feed|ac1-when-a-plugin-tool-action
flowchart LR
    Request[req_100_add_user_feedback_and_vs_code_noti] --> Problem[Give the operator immediate visible feedba]
    Problem --> Scope[Add plugin in-progress feedback for Ollama]
