...
# Problem
- Make the Logics workflow understandable through a dedicated onboarding webview that presents four visible steps: Need, Framing, Orchestration Tasks, and Execution.
...
- In:
  ...
  - surface the four-step onboarding model and the primary workflow actions in that webview
...
- Out:
  - redefining the four-step model itself beyond what `item_208` establishes
...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|add-the-three-step-onboarding-model-to-g|req-119-three-step-onboarding-for-need-f|make-the-logics-workflow-understandable-|ac1-the-product-exposes-four-clearly
flowchart LR
    Request[req_119_three_step_onboarding_for_need_fra] --> Problem[Make the Logics workflow understandable th]
    Problem --> Scope[Add the four step onboarding model]
    Scope --> Acceptance[AC1: The product exposes four clearly]
    Acceptance --> Tasks[task_109_orchestration_delivery_for_req_11]
```

# Acceptance criteria
- AC1: The product exposes four clearly labeled onboarding stages: Need, Framing, Orchestration Tasks, and Execution inside a dedicated onboarding webview.
...
# AC Traceability
- AC1 -> Webview implementation. Proof: render the four labeled stages in the onboarding webview using the model defined in `item_208`.
...
- AC4 -> Workflow alignment. Proof: validate that the visible onboarding still maps cleanly to canonical request, backlog, task, and orchestration-task behavior in the implementation.
...
# AI Context
- Summary: Implement the dedicated onboarding webview and its lifecycle so the four-step model and key actions appear once at first use or after a relevant update, then get out of the way.
