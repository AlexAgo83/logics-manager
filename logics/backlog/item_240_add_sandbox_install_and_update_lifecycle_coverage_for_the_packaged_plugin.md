...
- AC9 -> Scope: The request includes plugin lifecycle integration tests for packaged VSIX installs in a demo or sandbox workspace. These tests must cover at least fresh install and upgrade behavior for the plugin in a realistic local VS Code environment, must verify stable outcomes such as successful installation, activation, basic command or webview availability, and safe update behavior, and must remain opt-in or separately gated until their runtime cost and platform stability are well understood.. Proof: `tests/run_plugin_lifecycle_checks.mjs` implements 3 sandbox lifecycle tests (fresh install, update/reinstall, uninstall) using disposable `--extensions-dir` and `--user-data-dir`. Gated by `PLUGIN_LIFECYCLE_TESTS=1` and `code` CLI availability. Runs via `npm run test:lifecycle`. Skips cleanly with exit 0 when prerequisites are not met.
