## item_280_add_status_selector_button_ui_and_per_type_status_set_in_the_detail_panel - Add status selector button UI and per-type status set in the detail panel
> From version: 1.24.0
> Schema version: 1.0
> Status: Done
> Understanding: 100%
> Confidence: 100% (updated)
> Progress: 100%
> Complexity: Medium
> Theme: UI
...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|add-status-selector-button-ui-and-per-ty|req-154-add-a-manual-status-selector-but|add-a-button-in-the-detail|ac1-a-status-selector-button-appears
flowchart TD
    A[Detail Panel] --> B[Action Bar]
    B --> C[Status Selector Button]
    C --> D[Show Valid Statuses]
    D --> E[Highlight Current Status]
    E --> F[Update Status Indicator]
    F --> G[Trigger Board/List Refresh]
    B --> H[Done & Obsolete Buttons]
    C -.-> H
```
...
- Keywords: add, status, selector, button, and, per-type, set, the
- Use when: Use when implementing or reviewing the delivery slice for Add status selector button UI and per-type status set in the detail panel.
