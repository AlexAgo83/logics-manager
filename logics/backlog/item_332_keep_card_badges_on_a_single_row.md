## item_332_keep_card_badges_on_a_single_row - Keep card badges on a single row
> From version: 1.27.0
> Schema version: 1.0
> Status: Ready
> Understanding: 94%
...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|keep-card-badges-on-a-single-row|req-186-keep-card-badges-on-a-single-row|the-card-surfaces-currently-show-several|ac1-cards-that-already-show-multiple
flowchart TD
  A[Show badges on card] --> B[Check badge count]
  B -->|Few badges| C[Display badges in single row]
  B -->|Many badges| D[Apply compact overflow]
  C --> E[Maintain badge order and meaning]
  D --> E
  E --> F[Consistent layout across variants]
```
