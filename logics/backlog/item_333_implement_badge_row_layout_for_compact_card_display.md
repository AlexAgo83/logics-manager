...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|implement-badge-row-layout-for-compact-c|req-186-keep-card-badges-on-a-single-row|the-card-surfaces-currently-show-several|ac1-cards-that-already-show-multiple
flowchart TD
    A[Show badges on card] --> B[Keep badges on single row]
    B --> C{Row fits in width?}
    C -- Yes --> D[Display badges in order]
    C -- No --> E[Use compact overflow/truncation]
    D --> F[Preserve badge meanings]
    E --> F
    F --> G[Apply consistently across variants]
```
...
- Primary task(s): `task_143_implement_badge_row_layout_for_compact_card_display`
