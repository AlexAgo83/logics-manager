...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|define-badge-strip-styling-and-behavior-|req-186-keep-card-badges-on-a-single-row|the-card-surfaces-currently-show-several|ac1-cards-that-already-show-multiple
flowchart TD
    A[Show badges on one row] --> B[No wrapping on normal width]
    B --> C[Use compact overflow if space small]
    C --> D[Keep badge order and meaning]
    D --> E[Apply consistently to card variants]
```
