...
- Board chrome — topbar controls live in `clients/viewer/index.html` (`.toolbar__filters`, search `#search-input`, view-mode toggle `data-action="toggle-view-mode"`, Settings `#viewer-refresh-menu-button`); the filter panel is `#filter-panel`. The board/list toggle is the slider to relocate beside the search bar.
...
- AC4: The topbar exposes distinct Activity and Project entry points, left-hand controls appear/hide per active screen, and the board/list toggle becomes a slider beside the search bar with a defined mobile layout (search bar full-width on its own line, slider right-anchored on the controls line).
