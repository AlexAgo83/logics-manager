...
# Context
- **The MCP connector fails for a stated reason, and three layers destroy it.** Reproduced against a live viewer: `POST /api/mcp-connector` with `start` returns `running: true` with an empty URL, and twelve seconds later `status` returns `running: false` with `error` empty. Running the same command by hand shows why -- the tunnel exits immediately with `Port 8766 on 127.0.0.1 is already in use -- another logics-manager MCP server (or the viewer, on its own default port) is likely already running. Pass --port <n> for a different one.`
