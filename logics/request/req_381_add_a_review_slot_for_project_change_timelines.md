...
# Acceptance criteria
- AC1: A `Review` slot is reachable from the viewer's primary navigation without displacing existing Project, Activity, Workshop, Remote, CDX, Corpus, Settings, or Diagnostics access.
...
- AC10: Backend and browser-host tests cover burst construction, committed-file diff payloads, working-tree selection, keyboard navigation, and unavailable-state rendering.
- AC11: The generated browser host bundle is updated and `npm run bundle:viewer-host`, `npm run check:viewer-host`, targeted vitest/pytest checks, and `logics-manager lint --require-status` pass.
