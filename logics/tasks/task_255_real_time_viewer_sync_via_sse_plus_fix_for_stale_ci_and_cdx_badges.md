...
- [ ] Quick fix first: schedule `refreshBadgeCounters()` on the background tick so CI/CDX badges refresh without opening their screens (today only `refreshGitBadgeCounters()` runs when idle, `clients/viewer/browser-host.js:2547`).
...
- [ ] Tests (python + viewer) cover change-event/ETag/304 and client background badge refresh + SSE→poll fallback.
- [ ] `clients/viewer/` and `logics_manager/viewer_assets/viewer/` are byte-identical; full suite green.
