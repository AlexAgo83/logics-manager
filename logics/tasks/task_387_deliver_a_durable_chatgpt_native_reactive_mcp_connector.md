...
> Indicators reviewed: 2026-08-16 00:27:11
...
- [x] 4. Run focused MCP/viewer tests covering the tunnel launch path, the toggle/polling fixes, and the transport guidance; validate the request chain and record closeout evidence.
...
- pytest tests/python passed on 2026-08-16: 1442 passed; npx vitest run passed on 2026-08-16: 976 passed
- Finish workflow executed on 2026-08-16.
- Linked backlog/request close verification passed.

# Report
- 2026-08-16: all three slices delivered. item_849 (commit 3a1d171f) fixed the Settings toggle's field mismatch and made the connector screen poll itself. item_850/item_851 (commit 36f327c2) moved the ChatGPT path onto OpenAI's Secure MCP Tunnel -- logics_manager/mcp_tunnel.py builds the commands and reads tunnel-client doctor, the viewer supervises the child, installs the binary on confirmation, creates the profile, stores and checks the control-plane key, and the screen walks the five steps in order and names the transport per client class. Suites: 1442 python, 976 JS, all passing. Open ceiling: the key check is a short supervised tunnel-client run rather than a control-plane introspection call, and the connected flip is asserted at the viewer boundary only -- both marked in code.
- Finished on 2026-08-16.
- Linked backlog item(s): `item_849_fix_the_settings_connector_toggle_and_make_the_connector_screen_reactive`, `item_850_run_chatgpt_through_openais_secure_mcp_tunnel`, `item_851_tell_the_operator_which_transport_their_client_needs`
- Related request(s): `req_376_make_the_chatgpt_mcp_connector_plug_and_play`
