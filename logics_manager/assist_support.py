...
def _build_claude_instructions(repo_root: Path) -> dict[str, object]:
    content = "\n".join(
        [
            ...
            "",
            "When naming a Logics document in a response, give its viewer link too if one is running for this "
            "repository: `read_logics_doc` and the CLI's `flow show`/`sync list-docs`/`sync search-docs` already "
            "carry it, so this usually means passing along what they returned rather than building a link by hand. "
            "The address always comes from the running viewer, never a guessed default port; see `docs/cli.md` for "
            "the URL grammar.",
            "",
            ...
            "Not every fix needs a chain. ADR 030 decides which do: a fix needs one when its reasoning cannot live "
            "at the point where it would be undone -- it changes what the product asserts, trades one claim "
            "against another, rests on a measurement, or has a cause spanning layers. Otherwise the comment at "
            "that declaration and a test that fails without it are the record. Say which side a fix falls on when "
            "you fix it, not afterwards.",
