...
from . import mcp_project_context
...
TOOL_CAPABILITIES: dict[str, str] = {
    ...
    **{name: READ_ONLY for name in ("onboard_project", "list_projects", "get_active_project", "search_project_context", "read_project_resource")},
...
def _relative_path(repo_root: Path, raw_path: str, allowed_dirs: tuple[str, ...]) -> Path:
    ...
    try:
        relative_to_root(repo_root / normalized, repo_root)
    ...
    if has_symlink_segment(repo_root, normalized):
...
_TOOL_HANDLERS: dict[str, Any] = {
    ...
    **{"onboard_project": mcp_project_context.tool_onboard_project, "list_projects": mcp_project_context.tool_list_projects, "get_active_project": mcp_project_context.tool_get_active_project, "search_project_context": mcp_project_context.tool_search_project_context, "read_project_resource": mcp_project_context.tool_read_project_resource},
...
def call_tool(name: str, arguments: dict[str, Any] | None = None, *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    except mcp_project_context.ProjectContextError as exc:
        raise McpToolError(exc.code, exc.message, details=exc.details) from exc


def mcp_result(payload: dict[str, Any]) -> dict[str, Any]:
    return {"content": [{"type": "text", "text": json.dumps(payload, indent=2, sort_keys=True)}], "structuredContent": payload, "isError": not bool(payload.get("ok", True))}
