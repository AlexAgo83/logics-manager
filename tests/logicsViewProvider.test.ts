...
import * as viewProviderSupport from "../clients/vscode/src/logicsViewProviderSupport";
import { updateIndicatorsOnDisk } from "../clients/vscode/src/logicsProviderUtils";
...
describe("LogicsViewProvider", () => {
  ...
  it("uses a repair-oriented bootstrap prompt when logics exists but skills are missing", async () => {
    ...
    expect(mocks.showInformationMessage).toHaveBeenCalledWith(
      "Logics bootstrap is incomplete. Bootstrap or repair Logics by provisioning the local runtime?",
      "Bootstrap Logics",
      "Not now"
    );
  });
  ...
  it("warns once at startup when a broad gitignore pattern hides the bundled runtime", async () => {
    ...
    mocks.detectDangerousGitignorePatterns.mockReturnValueOnce({
      ...
      reason: "Broad .gitignore pattern(s) cover repo-local Logics runtime paths: logics/."
    });
    mocks.showInformationMessage.mockResolvedValue("Not now");
  ...
  it("uses the bundled runtime entrypoint on first bootstrap", async () => {
    mocks.inspectLogicsKitSubmodule.mockReturnValue({
      exists: false,
      isCanonical: false,
  ...
  it("auto-publishes the global kit during bootstrap before reporting full completion", async () => {
    ...
    expect(mocks.showInformationMessage).toHaveBeenCalledWith(
      "Logics bootstrapped. Repo-local runtime and the global Codex runtime are ready. Global Codex runtime publication completed during bootstrap. Refreshing."
    );
  });
});
