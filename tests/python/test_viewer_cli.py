...
def test_a_truncated_diff_says_the_rest_can_be_asked_for(tmp_path: Path) -> None:
    ...
    assert ["git", "diff", "--no-ext-diff", "--unified=5", "--", ":(literal)x"] in git_calls
    assert ["git", "diff", "--no-ext-diff", "--unified=80", "--", ":(literal)x"] in git_calls
...
def test_viewer_git_diff_payload_is_read_only_bounded_and_path_safe(tmp_path: Path) -> None:
    ...
    def runner(args: list[str], **_kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        if args[1:] == ["rev-parse", "--is-inside-work-tree"]:
            return subprocess.CompletedProcess(args, 0, "true\n", "")
        if args[1:] == ["diff", "--no-ext-diff", "--unified=5", "--cached", "--", ":(literal)logics/request/req_001_demo.md"]:
    ...
    assert ["git", "diff", "--no-ext-diff", "--unified=5", "--cached", "--", ":(literal)logics/request/req_001_demo.md"] in calls
    assert not any("push" in call or "fetch" in call or "pull" in call for call in calls for _ in [call])
...
def test_viewer_git_commit_diff_payload_can_scope_to_one_file(tmp_path: Path) -> None:
    ...
    def runner(args: list[str], **_kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        if args[1:] == ["rev-parse", "--is-inside-work-tree"]:
            return subprocess.CompletedProcess(args, 0, "true\n", "")
        if args[1:] == ["show", "--no-ext-diff", "--format=medium", "--stat", "--patch", "--find-renames", "--unified=5", "abc1234", "--", ":(literal)src/demo.ts"]:
    ...
    assert ["git", "show", "--no-ext-diff", "--format=medium", "--stat", "--patch", "--find-renames", "--unified=5", "abc1234", "--", ":(literal)src/demo.ts"] in calls
